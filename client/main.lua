...
RegisterNetEvent('qb-inventory:client:openInventory', function(items, other)
    ...
    if PlayerData.metadata and PlayerData.metadata.discord then
        discordName = PlayerData.metadata.discord.name or PlayerData.metadata.discord.displayName or "Unknown"
    else
        -- Fallback to old method if metadata is not yet synced
        for i = 0, GetNumPlayerIndices() - 1 do
            local identifier = GetPlayerIdentifier(PlayerId(), i)
            if identifier and string.find(identifier, "discord:") then
                discordName = identifier
                break
            end
        end
    end
